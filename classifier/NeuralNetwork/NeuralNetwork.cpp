...
//	QFile trainInspectFile("train.log");
//	QTextStream trainLogStream(&trainInspectFile);
//	if (!trainInspectFile.open(QIODevice::WriteOnly)) {
//		qDebug() << "failed to open train.log file";
//		return result;
//	}
...
//			int correct = 0;
//			for (int i = 0; i < train.size(); i++) {
//				const QVector<nnreal> was = classifyVec(train.at(i));
//				const QVector<nnreal> expected = exOutput.at(i);
//				const int wc = outputVectorToLabel(was);
//				const int ec = outputVectorToLabel(expected);
//				if (wc == ec) {
//					correct++;
//				}
//				qDebug() << i << "was" << was << "(" << wc << "), expected:" << expected << "(" << ec << ")";
//			}
//			qDebug() << "correct:" << nnreal(correct) / nnreal(train.size());
//			trainLogStream << "epoch " << epochNum << endl;
//			trainLogStream << *this << endl << endl;
		} else if (epochError > best * 3.0 / 0.1) {
			epochNum = maxEpochs;
		}
		result << epochError;
